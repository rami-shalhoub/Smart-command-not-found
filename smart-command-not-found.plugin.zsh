...
if command -v flatpak &>/dev/null; then
   ...
   [ -n "$flat_result" ] && echo "  ${fg[green]}✔ Found in Flatpak${reset_color}" && sources+=("Flatpak::$flat_result") && found=true
fi
...
manager=$(echo "$chosen" | sed -E 's/^.*?([A-Za-z]+) →.*/\1/')
package=$(echo "$chosen" | sed -E 's/^.*→[[:space:]]*([A-Za-z0-9._\/-]+).*/\1/')
